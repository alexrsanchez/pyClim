...
def quality_control(df,vars_to_check, t_units='C', wind_units='m/s', t_upper=50, t_lower=-50, wind_upper=200):
    '''
...
def plot_records_count(records_df, variable, database, station_name, filename, freq='day'):
    ...
    plt.legend(['Number of days above previous high record', 'Number of days below previous low record'], fontsize=14)
    plt.text(0.03, 0.955, 'Period with data: %i-%i' %(yearmin,yearmax), fontsize=14, transform=plt.gcf().transFigure)
    plt.text(0.8, 0.955, 'Database: %s' %database, fontsize=14, transform=plt.gcf().transFigure, wrap=True)
    plt.text(0.8, 0.915, 'Location: %s' %station_name, fontsize=14, transform=plt.gcf().transFigure, wrap=True)
    plt.tight_layout()
    fig.suptitle('%s records for %s in %s' %(title_var[freq], variable, station_name), fontsize=19, y=0.93)
    fig.savefig(filename,dpi=300)



def compute_and_plot_exceedances(df, variable, database, station_name, filename, threshold=0, time_scale='year', upwards=True, plot_means=False, averaging_period=5, alldatamean=False):
    '''
    ...
    if time_scale.lower() == 'year':
        ...
        if plot_means == True:
            ...
            if alldatamean == True:
                ax.hlines(exceedances.loc[:, variable].mean(), xmin=exceedances.Year.min(), xmax=exceedances.Year.max(), color="#FF0000", lw=1.3, label='Mean of all values')
        ...
        plt.text(0.03, 0.935, 'Period with data: %i-%i' %(yearmin,yearmax), fontsize=14, transform=plt.gcf().transFigure)
                ...
                if len(df_plot) % averaging_period == 1:
                    ...
                    for i in range(0,len(df_plot),averaging_period):
                        if i == 0:
                            ...
                            ax[j].hlines(df_plot.loc[i:min(i+averaging_period, len(df_plot)),variable].mean(), xmin=df_plot.Year[i], xmax=df_plot.Year[min(i+averaging_period, len(df_plot)-1)], color="#000000", lw=1.2, label='_nolegend_')
                if alldatamean == True:
                    ax[j].hlines(df_plot.loc[:, variable].mean(), xmin=df_plot.Year.min(), xmax=df_plot.Year.max(), color="#FF0000", lw=1.3, label='Mean of all values')
        ...
        #    ax[3].legend(fontsize=14,ncol=2)
            ax[2].legend(fontsize=16, ncol=3, loc='lower center', bbox_to_anchor=(1.03,-0.22), frameon=False) #, transform=plt.gcf().transFigure)

        ax[0].set_ylabel('exceedances', fontsize=15)           
        ax[2].set_ylabel('exceedances', fontsize=15)           


        plt.text(0.03, 0.96, 'Period with data: %i-%i' %(yearmin,yearmax), fontsize=14, transform=plt.gcf().transFigure)
        plt.text(0.8, 0.96, 'Database: %s' %database, fontsize=14, transform=plt.gcf().transFigure, wrap=True)
        plt.text(0.8, 0.925, 'Location: %s' %station_name, fontsize=14, transform=plt.gcf().transFigure, wrap=True)
        plt.subplots_adjust(bottom=0.05, left=0.07, right=0.98, hspace=0.18, wspace=0.12)
        ...
        for j in range(1,13):
            ...
            if plot_means == True:
                ...
                if alldatamean == True:
                    ax[j-1].hlines(df_plot.loc[:, variable].mean(), xmin=df_plot.Year.min(), xmax=df_plot.Year.max(), color="#FF0000", lw=1.3, label='Mean of all values')
        ...
        #    ax[11].legend(fontsize=14,ncol=2)
            ax[10].legend(fontsize=16, ncol=3, bbox_to_anchor=(0., -0.38, 1., .102), loc='lower center', frameon=False) #, transform=plt.gcf().transFigure)
        ...
        plt.text(0.03, 0.94, 'Period with data: %i-%i' %(yearmin,yearmax), fontsize=14, transform=plt.gcf().transFigure)
        plt.text(0.8, 0.97, 'Database: %s' %database, fontsize=14, transform=plt.gcf().transFigure, wrap=True)
        plt.text(0.8, 0.94, 'Location: %s' %station_name, fontsize=14, transform=plt.gcf().transFigure, wrap=True)
        plt.subplots_adjust(bottom=0.05, top=0.91, left=0.07, right=0.98, hspace=0.2, wspace=0.13)

        fig.suptitle('Monthly exceedances of %s=%1.f' %(variable,threshold), fontsize=24)
        fig.savefig(filename,dpi=300)
       
def plot_variable_trends(df, var, units, filename, database, station_name, averaging_period=5, grouping='year', grouping_stat='mean', rain_limit=1, plot_kind='line', alldatamean=True):
    '''
   ...
   '''
    ...
    if grouping.lower() == 'year':
        ...
        if plot_kind == 'bar':
            ax.bar(df.index, df[var], color="#008b4a", edgecolor='k', width=100, label=var, zorder=10)
        else:
        ...
        if alldatamean == True:
            ax.hlines(df.loc[:, var].mean(), xmin=df.index.min(), xmax=df.index.max(), color="#FF0000", lw=1.4, label='Mean of all values')
        ...
        plt.text(0.031, 0.955, 'Period with data: %i-%i' %(yearmin,yearmax), fontsize=14, transform=plt.gcf().transFigure)
        plt.text(0.78, 0.955, 'Database: %s' %database, fontsize=14, transform=plt.gcf().transFigure, wrap=True)
        plt.text(0.78, 0.915, 'Location: %s' %station_name, fontsize=14, transform=plt.gcf().transFigure, wrap=True)
        ...
        plt.subplots_adjust(left=0.07, right=0.98, hspace=0.1, wspace=0.1,bottom=0.075, top=0.87)
        #fig.autofmt_xdate()

        ax.tick_params(labelsize=17)
        ax.set_ylabel(units, fontsize=17)
        ax.grid(color='grey')
        ax.legend(fontsize=15, ncol=3, frameon=False, bbox_to_anchor=(-0.01,1.08), loc='upper left')
        ...
        for j in range(4):
            if plot_kind == 'bar':
                ax[j].bar(df_list[j][var].index, df_list[j][var].values, color="#008b4a", edgecolor='k', width=90, label=var, zorder=10)
            else:
                ax[j].plot(df_list[j][var].index, df_list[j][var].values, color="#008b4a", lw=1.4, label=var, zorder=10)
            ...
            if alldatamean == True:
                ax[j].hlines(df_list[j].loc[:, var].mean(), xmin=df_list[j].index.min(), xmax=df_list[j].index.max(), color="#FF0000", lw=1.4, label='Mean of all values')
        ...
        ax[2].legend(fontsize=14, ncol=3, bbox_to_anchor=(-0.05, -0.18, 1., .102), frameon=False)#, loc='lower center') #, transform=plt.gcf().transFigure)
        plt.text(0.03, 0.97, 'Period with data: %i-%i' %(yearmin,yearmax), fontsize=13, transform=plt.gcf().transFigure)
        plt.text(0.8, 0.97, 'Database: %s' %database, fontsize=13, transform=plt.gcf().transFigure, wrap=True)
        plt.text(0.8, 0.937, 'Location: %s' %station_name, fontsize=13, transform=plt.gcf().transFigure, wrap=True)
        ...
        plt.subplots_adjust(bottom=0.095, top=0.88, left=0.07, right=0.98, hspace=0.2, wspace=0.13)
        ...
        for j in range(1,13):
            if plot_kind == 'bar':
                ax[j-1].bar(df_list[j][var].index, df_list[j][var].values, color="#008b4a", edgecolor='k', width=28, label=var, zorder=10)
            else:
                ax[j-1].plot(df_list[j][var].index, df_list[j][var].values, color="#008b4a", lw=1.4, label=var, zorder=10)
            #if len(df_list[j]) % averaging_period == 1:
            for i in range(0,len(df_list[j])-1,averaging_period):
                if i == 0:
            ...
            if alldatamean == True:
                ax[j-1].hlines(df_list[j].loc[:, var].mean(), xmin=df_list[j].index.min(), xmax=df_list[j].index.max(), color="#FF0000", lw=1.4, label='Mean of all values')
        ...
        ax[10].legend(fontsize=14, ncol=3, bbox_to_anchor=(0., -0.44, 1., .102), loc='lower center', frameon=False) #, transform=plt.gcf().transFigure)

        plt.text(0.03, 0.9675, 'Period with data: %i-%i' %(yearmin,yearmax), fontsize=14, transform=plt.gcf().transFigure)
        plt.text(0.8, 0.9675, 'Database: %s' %database, fontsize=14, transform=plt.gcf().transFigure, wrap=True)
        plt.text(0.8, 0.938, 'Location: %s' %station_name, fontsize=14, transform=plt.gcf().transFigure, wrap=True)
...
#        plt.suptitle('%s evolution [%s] and %i-period mean' %(var, units, averaging_period), fontsize=20, y=0.97)
        plt.subplots_adjust(left=0.05, right=0.975, hspace=0.2, wspace=0.135,bottom=0.095, top=0.9)
...
#    fig.autofmt_xdate()
    ...
    plt.text(0.03, 0.925, 'Period with data: %i-%i' %(yearmin, yearmax), fontsize=12, transform=plt.gcf().transFigure)
    plt.text(0.825, 0.955, 'Database: %s' %database, fontsize=12, transform=plt.gcf().transFigure, wrap=True)
    plt.text(0.825, 0.925, 'Location: %s' %station_name, fontsize=12, transform=plt.gcf().transFigure, wrap=True)
...
def plot_data_vs_climate_withrecords(df,df_climate,records_df,variable,units,inidate,enddate,colormap,database,climate_normal_period, station_name, filename, kind='line', climate_stat='median', fillcolor_gradient=False, use_std=True, show_bands = True, show_seasons=True):
    ...
    plt.text(0.03, 0.955, 'Climate normal period: %i-%i' %(climate_normal_period[0],climate_normal_period[1]), fontsize=12, transform=plt.gcf().transFigure)
    plt.text(0.03, 0.915, 'Period with data: %i-%i' %(yearmin, yearmax), fontsize=12, transform=plt.gcf().transFigure)
    plt.text(0.825, 0.955, 'Database: %s' %database, fontsize=12, transform=plt.gcf().transFigure, wrap=True)
    plt.text(0.825, 0.915, 'Location: %s' %station_name, fontsize=12, transform=plt.gcf().transFigure, wrap=True)
...
#        ax.plot(df_climate.index, np.tile(df_climate[var+'_%s' %stat].median(),len(df_climate)),color="#47fe60",label='Median: %.1f%s' %(df_climate[var+'_%s' %stat].median(), units), zorder=2)
    ...
    ax.add_artist(text)
    fig.autofmt_xdate()
    plt.text(0.825, 0.935, 'Database: %s' %database, fontsize=12, transform=plt.gcf().transFigure, wrap=True)
    plt.text(0.825, 0.905, 'Location: %s' %station_name, fontsize=12, transform=plt.gcf().transFigure, wrap=True)
...
def plot_data_and_yearly_cycle(df,df_climate,year_to_plot,vars_list,units_list,colormap,database, climate_normal_period, station_name, plotdir, climate_stat='median', fillcolor_gradient=False, show_seasons=True):
    ...
    for i in range(len(vars_list)):
        ...
        if units_list[i] not in ['mm', 'in', 'cm']:
            ...
            ax[1].text(x=0.7, y=0.05, s='Last accumulated anomaly: %.1f %s' %(accum_mean_all.dropna(subset=['anomaly']).iloc[-1,:]['anomaly'], units_list[i]), transform=ax[1].transAxes, fontsize=14, bbox=props)
            ...
            # these are matplotlib.patch.Patch properties
            props = dict(boxstyle='round', facecolor='wheat', alpha=0.5)
            ax[1].text(x=0.5, y=0.04, s='Last accumulated anomaly: %.1f' %cum_df.dropna(subset=['anomaly']).iloc[-1,:]['anomaly'], transform=ax[1].transAxes, fontsize=14, bbox=props)
...
def timeseries_extremevalues(df, var, units, climate_normal_period, database, station_name, filename, time_scale='Year'):
    ...
    if time_scale.lower() == 'year':
        ...
        plt.text(0.03, 0.92, 'Period with data: %i-%i' %(yearmin,yearmax), fontsize=14, transform=plt.gcf().transFigure)
        plt.text(0.8, 0.96, 'Database: %s' %database, fontsize=14, transform=plt.gcf().transFigure, wrap=True)
        plt.text(0.8, 0.92, 'Location: %s' %station_name, fontsize=14, transform=plt.gcf().transFigure, wrap=True)
        ...
        plt.text(0.03, 0.96, 'Climate normal period: %i-%i' %(climate_normal_period[0],climate_normal_period[1]), fontsize=14, transform=plt.gcf().transFigure)
        plt.text(0.03, 0.925, 'Period with data: %i-%i' %(yearmin,yearmax), fontsize=14, transform=plt.gcf().transFigure)
        plt.text(0.8, 0.96, 'Database: %s' %database, fontsize=14, transform=plt.gcf().transFigure, wrap=True)
        plt.text(0.8, 0.925, 'Location: %s' %station_name, fontsize=14, transform=plt.gcf().transFigure, wrap=True)
...
#                ax.plot(np.arange(0,len(df1_complete_join.loc[df1_complete_join.index.year == annual_vals[sorted(list(set(df1_complete_join.columns) & set(variables)), key=lambda x: variables.index(x))[i]].sort_values().index[-3:][ye]].index),1), df1_complete_join.loc[df1_complete_join.index.year == annual_vals[sorted(list(set(df1_complete_join.columns) & set(variables)), key=lambda x: variables.index(x))[i]].sort_values().index[-3:][ye], sorted(list(set(df1_complete_join.columns) & set(variables)), key=lambda x: variables.index(x))[i]],color=colores[ye], label=df1_complete_join[df1_complete_join.index.year.isin(annual_vals[sorted(list(set(df1_complete_join.columns) & set(variables)), key=lambda x: variables.index(x))[i]].sort_values().index[-3:])].index.year.unique()[ye], lw=1.1, zorder=1)
    ...
    if year_to_plot in df.index.year.unique():
        ...
        ax.plot(np.arange(0,len(df.loc[(df.index.year == year_to_plot) & ~((df.index.month == 2) & (df.index.day == 29)), variable].index),1), df.loc[(df.index.year == year_to_plot) & ~((df.index.month == 2) & (df.index.day == 29)),'%s_median' %variable],color='k',label='Median [%i-%i]' %(climate_normal_period[0],climate_normal_period[1]),zorder=3)
...
def annual_meteogram(df, df_climate, year_to_plot, climate_normal_period, database, station_name, filename,plot_anoms=False, show_seasons=True):
    ...
    text.patch.set_alpha(0.5)
    plt.text(0.031, 0.955, 'Climate normal period: %i-%i' %(climate_normal_period[0],climate_normal_period[1]), fontsize=16, transform=plt.gcf().transFigure)
    plt.text(0.031, 0.93, 'Period with data: %i-%i' %(yearmin, yearmax), fontsize=16, transform=plt.gcf().transFigure)
    plt.text(0.79, 0.955, 'Database: %s' %database, fontsize=16, transform=plt.gcf().transFigure, wrap=True)
    plt.text(0.79, 0.93, 'Location: %s' %station_name, fontsize=16, transform=plt.gcf().transFigure, wrap=True)
    plt.subplots_adjust(left=0.05, right=0.95, hspace=0.12, wspace=0.1,bottom=0.03, top=0.9)
    #fig.autofmt_xdate()
    fig.savefig(filename,dpi=300)
...
#        df_year[var+'_cumanom'] = df_year['anom_cumsum'] / (df_year.index + 1)
    ...
    plt.text(0.03, 0.93, 'Period with data: %i-%i' %(yearmin,yearmax), fontsize=12, transform=plt.gcf().transFigure)
...
#    ax.set_xlim([0,max(df_year.index)])
    ...
    plt.text(0.03, 0.95, 'Climate normal period: %i-%i' %(climate_normal_period[0],climate_normal_period[1]), fontsize=12, transform=plt.gcf().transFigure)
    plt.text(0.03, 0.91, 'Period with data: %i-%i' %(yearmin,yearmax), fontsize=12, transform=plt.gcf().transFigure)
    plt.text(0.77, 0.95, 'Database: %s' %database, fontsize=12, transform=plt.gcf().transFigure, wrap=True)
    plt.text(0.77, 0.91, 'Location: %s' %station_name, fontsize=12, transform=plt.gcf().transFigure, wrap=True)
...
def compare_with_globaldataset(df, var, units, database, station_name, filename, window=None, global_dataset='HadCRUT'):
    ...
    if len(normal_values) == 0:
        print('Your data has not data within the period 1961-1990. Therefore, there cannot be perform a comparison with the global dataset. Exiting...')
        return
...
def categories_evolution(df, var, units, categories_breaks, categories_labels, categories_colors, database, station_name, filename, time_scale='year', grouping_stat='mean'):
    '''
   ...
   '''
    ...
    if len(categories_labels) != len(categories_breaks) - 1:
        print('Check your categories lists. The length of the labels list should be one less than the length of the categories values')
    ...
    if len(categories_labels) == 0:
        ...
        for i in range(1,len(categories_breaks)):
            categories_labels.append('%s-%s' %(categories_breaks[i-1], categories_breaks[i]))

    if len(categories_colors) < len(categories_labels):
        raise ValueError('The number of colors is lower than the number of labels. Please, add more colors.')
    ...
    if time_scale.lower() == 'year':
        ...
        for i in range(len(binned_list)):
            ...
            binned_list[i]['%s' %year] = binned_list[i][var].map(binned_list_freq.squeeze())
        ...
        #    bottom += weight_count
        binned_df.plot.bar(stacked=True, color=categories_colors, ax=ax, width=0.99, zorder=100, alpha=0.7)
        text = AnchoredText('Alejandro Rodríguez Sánchez',
                        loc=1,bbox_to_anchor=(0.24, 0.185),
                        bbox_transform=ax.transAxes, prop={'size': 12}, frameon=True)
        text.patch.set_alpha(0.5)
        ...
        #plt.suptitle('%s evolution and %i-period mean' %(var, averaging_period), fontsize=22, y=0.97)
        plt.suptitle('%s [%s] grouped by categories' %(var, units), fontsize=22, y=0.97)
        plt.subplots_adjust(left=0.07, right=0.98, hspace=0.1, wspace=0.1,bottom=0.075, top=0.85)
        #fig.autofmt_xdate()

        ax.tick_params(labelsize=17, rotation=0)
        ax.set_ylabel('Frequency [%]', fontsize=17)
        ax.grid(color='grey')
        ax.legend(fontsize=15, ncol=len(categories_labels), loc='upper left', bbox_to_anchor=(0.0,1.1), frameon=False).set_zorder(101)
        ax.set_ylim([0,100])
    ...
    if time_scale.lower() == 'season':
        ...
        df_winter = binned[binned.season == 'DJF']#.groupby(['Year']).apply(grouping_stat, numeric_only=True)
        df_spring = binned[binned.season == 'MAM']#.groupby(['Year']).apply(grouping_stat, numeric_only=True)
        df_summer = binned[binned.season == 'JJA']#.groupby(['Year']).apply(grouping_stat, numeric_only=True)
        df_autumn = binned[binned.season == 'SON']#.groupby(['Year']).apply(grouping_stat, numeric_only=True)
        ...
        for j in range(4):
            ...
            binned_df.plot.bar(stacked=True, color=categories_colors, ax=ax[j], width=0.99, zorder=100, alpha=0.7)
            text = AnchoredText('Alejandro Rodríguez Sánchez',
                            loc=1,bbox_to_anchor=(0.24, 0.185),
                            bbox_transform=ax[j].transAxes, prop={'size': 12}, frameon=True)
            text.patch.set_alpha(0.5)
            ...
            ax[j].set_title(['December-January-February', 'March-April-May', 'June-July-August', 'September-October-November'][j], fontsize=17)

            if j == 1:
                ax[j].legend(fontsize=15, ncol=len(categories_labels), loc='lower left', bbox_to_anchor=(-1.12,1.055), frameon=False).set_zorder(101)
            else:
        ...
        plt.text(0.78, 0.925, 'Location: %s' %station_name, fontsize=14, transform=plt.gcf().transFigure)
        #plt.suptitle('%s evolution and %i-period mean' %(var, averaging_period), fontsize=22, y=0.97)
        plt.suptitle('%s [%s] grouped by categories' %(var, units), fontsize=24, y=0.975)
        plt.subplots_adjust(left=0.07, right=0.98, hspace=0.2, wspace=0.1,bottom=0.05, top=0.85)
    ...
    if time_scale.lower() == 'month':
        ...
        fig, axs = plt.subplots(figsize=(15.5,12.25), ncols=3, nrows=4, sharex=True, sharey=True)
        ax = axs.flatten()
        for j in range(12):
            binned_list = [g for n, g in df_list[j+1].groupby(pd.Grouper(freq='YS'))] #binned.groupby(pd.Grouper(freq='AS')).apply(grouping_stat, numeric_only=True)
...
#                binned_df = pd.concat([binned_df, binned_list[i]['%s' %year]], axis=1)
            ...
            binned_df.index = binned_df.index.astype(int) #pd.to_datetime(binned_df.index)

            binned_df.plot.bar(stacked=True, color=categories_colors, ax=ax[j], width=0.991, zorder=100, alpha=0.7)
            text = AnchoredText('Alejandro Rodríguez Sánchez',
                            loc=1,bbox_to_anchor=(0.24, 0.185),
                            bbox_transform=ax[j].transAxes, prop={'size': 12}, frameon=True)
            text.patch.set_alpha(0.5)
            ...
            if j == 0:
                ax[j].legend(fontsize=14, ncol=len(categories_labels), loc='lower left', bbox_to_anchor=(.01,1.075), frameon=False).set_zorder(101)
            else:
                ax[j].legend().set_visible(False)
        ...
        plt.text(0.031, 0.96, 'Period with data: %i-%i' %(yearmin,yearmax), fontsize=14, transform=plt.gcf().transFigure)
        plt.text(0.78, 0.96, 'Database: %s' %database, fontsize=14, transform=plt.gcf().transFigure)
        plt.text(0.78, 0.94, 'Location: %s' %station_name, fontsize=14, transform=plt.gcf().transFigure)
        #plt.suptitle('%s evolution and %i-period mean' %(var, averaging_period), fontsize=22, y=0.97)
        plt.suptitle('%s [%s] grouped by categories' %(var,units), fontsize=24, y=0.975)
        plt.subplots_adjust(left=0.07, right=0.98, hspace=0.2, wspace=0.12,bottom=0.035, top=0.89)
        fig.savefig(filename,dpi=300)