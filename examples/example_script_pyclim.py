...
yearinicio = int(df1.Year.values[0])
...
print(df1)
print(df1.iloc[-1,:]['Rainfall'])


climate_vars = ['Tmax','Tmean','Tmin','Rainfall','Accumpcp','Accumpcphidro','WindSpeed','WindMaxSpeed']

df1_complete = df1.reindex(pd.date_range('%i-01-01' %yearinicio, '%i-12-31' %yeartoday, freq='1D')) # Fill data in case there is any missing date
...
plot_accumulated_anomalies(df1_complete, 'Tmean', 'ºC', 2025, climate_normal_period, database, station_name, plotdir+'/Tmean_accum_anoms_monthly.png',freq='1ME')    
...
for var in list(set(df1_complete.columns) & set(variables)):
    ...
    plot_periodaverages(df1_complete,climate_df_sep, var, units, dt.datetime(2025,6,1), dt.datetime(2025,9,1), station_name, database, plotdir, stat='median', window=10)
...
plot_records_count(multiyearrecords_df_allvars, 'Tmean', database, station_name, plotdir+'/monthly_records_Tmean.png', freq='month') # Plot number of days exceeding daily records
...
plot_data_vs_climate_withrecords_multivar(df1_complete,climate_df_sep,multiyearrecords_df_allvars,varis,units_varis,inidate,enddate,cmap_anom_bars,database,climate_normal_period, station_name, plotdir+'/multivar%s_Rainfall_timeseries_climatemedian19912020_withrecords.png' %varis[i],
                                            kind='bar', fillcolor_gradient=False)
plot_data_vs_climate_withrecords_multivar(df1_complete,climate_df_sep,multiyearrecords_df_allvars,varis,units_varis,inidate,enddate,cmap_anom_bars,database,climate_normal_period, station_name, plotdir+'/multivar%s_Rainfall_timeseries_climatemedian19912020_withrecords_std.png' %varis[i], 
                                            kind='bar', fillcolor_gradient=False, use_std=True)
plot_data_vs_climate_withrecords_multivar(df1_complete,climate_df_sep,multiyearrecords_df_allvars,varis,units_varis,inidate,enddate,cmap_anom_bars,database,climate_normal_period, station_name, plotdir+'/multivar%s_Rainfall_timeseries_climatemedian19912020_withrecords_std_line.png' %varis[i], 
...
for var in sorted(list(set(df1_complete.columns) & set(variables)), key=lambda x: variables.index(x)):
    ...
    categories_evolution(df1_complete, 'Tmean', 'ºC', categories, [str(x) for x in range(1,len(categories))], colors, database, station_name, plotdir+'/categories_Tmean.png', time_scale='year')
    categories_evolution(df1_complete, 'Tmean', 'ºC', categories, [], colors, database, station_name, plotdir+'/categories_Tmean_default.png', time_scale='year')
    categories_evolution(df1_complete, 'Tmean', 'ºC', categories, [], colors, database, station_name, plotdir+'/categories_Tmean_season_default.png', time_scale='season')
    categories_evolution(df1_complete, 'Tmean', 'ºC', categories, [], colors, database, station_name, plotdir+'/categories_Tmean_month_default.png', time_scale='month')

    categories =  [0,1,5,10,15,20,25,30]
    colors = ['lightgray','blue','cyan','green','yellow','orange','red']
    categories_evolution(df1_complete, 'Rainfall', 'mm', categories, [str(x) for x in range(1,len(categories))], colors, database, station_name, plotdir+'/categories_rainfall.png', time_scale='year')
    categories_evolution(df1_complete, 'Rainfall', 'mm', categories, [], colors, database, station_name, plotdir+'/categories_rainfall_default.png', time_scale='year')
    categories_evolution(df1_complete, 'Rainfall', 'mm', categories, [], colors, database, station_name, plotdir+'/categories_rainfall_season_default.png', time_scale='season')
    categories_evolution(df1_complete, 'Rainfall', 'mm', categories, [], colors, database, station_name, plotdir+'/categories_rainfall_month_default.png', time_scale='month')
